...
const UpdateSolution = ({ updateModal, setUpdateModal, mutate }: any) => {
    ...
    const updatePortfolioHandler = (data: any) => {
        ...
        fetch(`${process.env.BASE_URL}/api/solution?id=${updateModal?.id}`, {
            method: "PUT",
            body: formData,
        })
            .then((res) => res.json())
            ...
            .catch((errors) => {
                toast.error("Failed to Solution");
                ...
                >
                    ...
                    <div className=" w-full h-[250px] py-2 overflow-hidden">
                        <Image
                            ...
                            layout="responsive"
                            height={80}
                            src={`/uploads/solution/${updateModal?.image}`}
                            alt="image"
                        />
                    </div>
                    <div className="relative  rounded-lg mt-4 ">
